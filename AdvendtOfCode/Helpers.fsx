...
module FloydWarshall =
    let floydWarshall (vCount : int) (edges : ((int*int)*int) array) =
        let dist = Array2D.create vCount vCount 1_000_000
        ...
        for k in 0 .. (vCount-1) do
            for i in 0 .. (vCount-1) do
                for j in 0 .. (vCount-1) do
                    let candidateDist = dist[i,k] + dist[k,j]
                    if (dist[i,j] > candidateDist) then
